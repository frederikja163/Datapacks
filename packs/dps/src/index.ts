...
export function build(): Datapack {
  ...
  const fallGuard = {
    ...
    levelup: d.defineFunction("powerup/fall_guard/levelup", [
      ...
      "$execute if score @s dps_acrobatics_cooldown_max matches ..0 run scoreboard players set @s dps_acrobatics_cooldown_max 0",
    ...
    apply: d.defineFunction("powerup/fall_guard/apply", [
      ...
      "# One charge saves one whole heart; sub-heart falls cost nothing.",
      "scoreboard players operation __fall_hearts__ dps_tmp = __fall__ dps_tmp",
      "scoreboard players operation __fall_hearts__ dps_tmp /= 10 dps_globals",
      "execute if score __fall_hearts__ dps_tmp matches ..0 run return fail",
      "# Needs a charge.",
      "execute if score @s dps_acrobatics_charges matches ..0 run return fail",
      "",
      "# Spend one charge per heart, up to the charges available.",
      "scoreboard players operation __fall_shield__ dps_tmp = @s dps_acrobatics_charges",
      "execute if score __fall_shield__ dps_tmp > __fall_hearts__ dps_tmp run scoreboard players operation __fall_shield__ dps_tmp = __fall_hearts__ dps_tmp",
      "scoreboard players operation @s dps_acrobatics_charges -= __fall_shield__ dps_tmp",
      "scoreboard players set @s dps_acrobatics_guard 1",
      `tellraw @s ${snbt([
        text("Charged Fall Guard", { color: "gold" }),
        text(" • ", { color: "dark_gray" }),
        text("Saved ", { color: "gold" }),
        score("__fall_shield__", "dps_tmp", { color: "aqua" }),
        text(" ❤", { color: "red" }),
      ])}`,
      "",
      "# Reduction in tenths = hearts saved * 10.",
      "scoreboard players operation __fall_reduce__ dps_tmp = __fall_shield__ dps_tmp",
      "scoreboard players operation __fall_reduce__ dps_tmp *= 10 dps_globals",
      "",
    ...
    second: d.defineFunction("powerup/fall_guard/second", [
      "$execute as @a[scores={$(skill)_cooldown=1..}] if score @s $(skill)_charges < @s $(skill)_charges_max run scoreboard players remove @s $(skill)_cooldown 1",
      "$execute as @a if score @s $(skill)_cooldown > @s $(skill)_cooldown_max run scoreboard players operation @s $(skill)_cooldown = @s $(skill)_cooldown_max",
      "$execute as @a[scores={$(skill)_cooldown=..0}] if score @s $(skill)_charges < @s $(skill)_charges_max run scoreboard players add @s $(skill)_charges 1",
      "$execute as @a[scores={$(skill)_cooldown=..0}] if score @s $(skill)_charges < @s $(skill)_charges_max run scoreboard players operation @s $(skill)_cooldown = @s $(skill)_cooldown_max",
    ]),
